...
import { GetUniversitiesByTypeType } from "@cust-types/admin/universityTypes";
...
export const ListUniversities = ({ name, university }: Props) => {
  ...
  const { loading: loadingRegions, data: dataRegions } = useQuery<GetAllRegionsType>(GET_REGIONS);
  const { loading: loadingUniversities, data: dataUniversities, refetch: refetchUniversities } = useQuery<GetUniversitiesByTypeType>(GET_UNIVERSITIES_BY_TYPE, {
